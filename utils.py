...
def load_model_and_may_interpolate(ckpt_path, model, model_key, model_prefix):
    ...
    keys = list(checkpoint_model.keys())
    for key in keys:
        #TODO: take care of position embedding shape when load from trained model
        if ".A." in key:
            weight_A = checkpoint_model[key]
            weight_B = copy.deepcopy(weight_A)
            weight_C = copy.deepcopy(weight_A)
            key_B = key.replace('.A.', '.B.')
            key_C = key.replace('.A.', '.C.')
            checkpoint_model[key_B] = weight_B
            checkpoint_model[key_C] = weight_C
...
def load_model_and_may_interpolate_from_trained_model(ckpt_path, model, model_key, model_prefix):
    if ckpt_path.startswith('https'):
        checkpoint = torch.hub.load_state_dict_from_url(
            ckpt_path, map_location='cpu', check_hash=True)
    else:
