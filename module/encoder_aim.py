...
# ref to https://github.com/chaoyi-wu/RadFM
...
class EncoderLayer(nn.Module):
    def __init__(self, args, depth, is_moe_layer=False, is_encoder_decoder=False, scale=1., 
        ...
        self.MLP_Adapter = MultiwayWrapper(
                args,
                Adapter(self.embed_dim, skip_connect=False)
            )
        self.S_Adapter = MultiwayWrapper(
            args,
            Adapter(self.embed_dim)
        )
    ...
    def forward(self, x, encoder_padding_mask, attn_mask=None, rel_pos=None, 
        ...
        if self.split_attn:
            ...
            a = torch.bmm(a.unsqueeze(2), a.unsqueeze(1))
        ...
        #x = x + self.scale*self.MLP_Adapter(x)   # spatial adater
        #x = self.scale*self.MLP_Adapter(x)   # spatial adater
...
class Encoder_Aim(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        print('!!! Using AIM-based adapter for encoder')
